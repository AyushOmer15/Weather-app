...
const weathCode=(longitude,latitude,callback)=>{
...
else if(response.body.error)
 {
     ...
     callback(undefined,`Weather in ${response.body.location.name} is ${response.body.current.weather_descriptions[0]} with Current temperature ${response.body.current.temperature} but it feels like ${response.body.current.feelslike} humidity is ${response.body.current.humidity}`)
 }
    })
